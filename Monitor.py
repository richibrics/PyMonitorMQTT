...
class Monitor():
    ...
    def Setup(self):
        ...
        if CONFIG_SENSORS_KEY in self.config:
            self.LoadEntities(self.config[CONFIG_SENSORS_KEY],SENSOR_NAME_SUFFIX,SENSORS_MODULE_NAME)
        if CONFIG_COMMANDS_KEY in self.config:
            self.LoadEntities(self.config[CONFIG_COMMANDS_KEY],COMMAND_NAME_SUFFIX,COMMAND_MODULE_NAME)
